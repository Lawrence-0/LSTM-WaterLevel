...
import pycodes.dias_obsv as dias_obsv
import pandas
...
def make_data1(obsvId):
    ...
    for r in obsvRain:
        if len(mlit_obsv._obsv_data(r, '20100101', '20191231')) == 87648:
            names.append(r)
            datumR = dias_obsv._obsv_data(r, '20310101', '20591231')
            trainingData.append(datumR)
    trainingData = pandas.DataFrame(trainingData).T
    trainingData.columns = names
    return trainingData
...
def make_dias(obsvId):
    assert len(mlit_obsv._obsv_data(obsvId, '20100101', '20110831')) == 14592
    posXY = dias_tools.findPos(obsvId)
    trainingData = []
    names = []
    names.append(obsvId)
    datumW = mlit_obsv._obsv_data(obsvId, '20100101', '20110831')
    for index in range(len(datumW)):
        if datumW[index] == float("-inf") or datumW[index] == float("inf") or datumW[index] == float("nan"):
            if index != 0:
                datumW[index] = datumW[index - 1]
    ...
    trainingData.append(datumW)
    for pos in posXY:
        if len(dias_tools._dias_data(pos[0], pos[1], '20100101', '20110831')) == 14592:
            names.append('X' + str(pos[0]) + '_Y' + str(pos[1]))
            trainingData.append(dias_tools._dias_data(pos[0], pos[1], '20100101', '20110831'))
    trainingData = pandas.DataFrame(trainingData).T
    trainingData.columns = names
    return trainingData

def make_dias1(obsvId):
    posXY = dias_tools.findPos(obsvId)
    trainingData = []
    names = []
    for pos in posXY:
        names.append('X' + str(pos[0]) + '_Y' + str(pos[1]))
        trainingData.append(dias_tools._dias_data1(pos[0], pos[1], '20310101', '20591231'))
    trainingData = pandas.DataFrame(trainingData).T
    trainingData.columns = names
    return trainingData